...
namespace DbManager
{
    public class MiniSQLParser
    {
        public static MiniSqlQuery Parse(string miniSQLQuery)
        {
            ...
            const string addUserPattern = @"^ADD\s+USER\s*\(\s*(?<username>[a-zA-Z]+)\s*,\s*(?<password>[a-zA-Z]+)\s*,\s*(?<profile>[a-zA-Z]+)\s*\)\s*;?\s*$";

            const string deleteUserPattern = @"^DELETE\s+USER\s*\(\s*(?<username>[a-zA-Z]+)\s*\)\s*;?\s*$";
